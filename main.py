...
def main():
    key = "your_encryption_key"  # Replace with an actual encryption key
    res = HD  # Or use four_k, depending on what you want

    encode("data/test.mp3", key, res)
    decode("test.mp3.gif", key, res)

    encode("data/test.txt", key, res)
    decode("test.txt.gif", key, res)
    
    encode("data/test.jpg", key, res)
    decode("test.jpg.gif", key, res)
